...
const bootstrap = async () => {
  console.log('[数据探查中心] bootstrap');
  return Promise.resolve();
};

const mount = async (props) => {
  console.log('[数据探查中心] mount', props);
  ...
  if (window.__POWERED_BY_QIANKUN__) {
    ...
    clientScript.src = '//localhost:5174/@vite/client';
    clientScript.type = 'module';
    document.head.appendChild(clientScript);
    
    // 加载应用主脚本
    const mainScript = document.createElement('script');
    mainScript.src = '//localhost:5174/src/main.ts';
...
const update = async (props) => {
  console.log('[数据探查中心] update', props);
  // 触发自定义事件，通知子应用更新
...
window['data-explore'] = window.dataExplore; 
