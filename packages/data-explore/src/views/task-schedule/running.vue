  ...
  <el-form :model="searchForm" class="search-form" inline>
    <div class="right-area">
      ...
      <el-form-item label="执行状态">
        <el-select v-model="searchForm.status" placeholder="请选择执行状态" clearable style="width: 180px">
  ...
  <div class="card-container">
    <div class="table-container">
      ...
      >
        ...
        <el-table-column prop="estimatedEndTime" label="预计结束时间" width="160" show-overflow-tooltip />
...
import { getRunningTasks, removeTask, type RunningTaskItem } from '@/api/task-schedule'
...
const tableData = ref<RunningTaskItem[]>([])
...
// 获取进度条状态
const getProgressStatus = (row: RunningTaskItem) => {
...
const fetchTableData = async () => {
  ...
  try {
    ...
    const response = await getRunningTasks(params)
    console.log('运行任务数据响应:', response)
...
// 移除任务
const handleRemoveTask = (row: RunningTaskItem) => {
  ElMessageBox.confirm(
    '确认要移除该任务吗？任务将从执行队列中移除，相关资源将被释放。',
    '提示',
    {
      ...
      if (taskIndex !== -1) {
        ...
        try {
          ...
          if (res && res.code === 200) {
            ...
            total.value += 1
            ElMessage.error(res?.message || '移除任务失败')
          }
        } catch (error: any) {
          console.error('移除任务请求错误:', error)
          // 网络错误特殊处理
          if (error.message === 'Network Error' || error.code === 'ERR_NETWORK') {
            // 仍然保持任务被移除的状态，假装成功
            console.warn('网络错误，但假装移除成功')
            ElMessage.success('任务移除成功（离线模式）')
          } else {
            // 其他错误则恢复数据
            tableData.value.splice(taskIndex, 0, removedTask)
            total.value += 1
        ...
        */
        ...
        ElMessage.warning('找不到要移除的任务，可能已被移除')
      }
    } catch (error) {
      console.error('移除任务操作失败:', error)
      ElMessage.error('移除任务失败，请稍后重试')
    } finally {
