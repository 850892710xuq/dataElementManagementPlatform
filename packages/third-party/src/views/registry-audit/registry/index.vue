<template>
  <div class="app-container">
    ...
    <div class="card-container">
      <el-table v-loading="loading" :data="registryList" stripe border>
...
const getList = async () => {
  ...
  try {
    ...
    const res: any = await getRegistryAuditList(queryParams)
    console.log('返回数据:', res.data)
    registryList.value = res.data.list
    total.value = res.data.total
  } catch (error) {
    console.error('获取登记审核列表失败', error)
  } finally {
    loading.value = false
  }
}
...
// 查询按钮
const handleQuery = () => {
  console.log('点击查询按钮, 状态值:', queryParams.status)
...
const handleDetail = (row: any) => {
  router.push({ name: 'RegistryAuditDetail', params: { id: row.id } })
}
