<template>
  <div class="app-container">
    ...
    <div class="card-container">
      <el-table v-loading="loading" :data="cancelList" stripe border>
        ...
        <el-table-column prop="cancelReason" label="注销原因" min-width="200" show-overflow-tooltip />
    ...
    <el-dialog title="注销登记审核" v-model="auditVisible" width="500px" append-to-body>
      <el-form ref="auditFormRef" :model="auditForm" label-width="100px">
        ...
        <el-form-item label="注销原因">
          <div style="white-space: pre-line;">{{ (currentRow as any).cancelReason }}</div>
        </el-form-item>
...
import { getCancelRegistryList, auditRegistry } from '@/api/registry-audit'
...
const getList = async () => {
  ...
  try {
    ...
    const res: any = await getCancelRegistryList(queryParams)
    console.log('返回数据:', res.data)
    cancelList.value = res.data.list
    total.value = res.data.total
  } catch (error) {
    console.error('获取注销登记审核列表失败', error)
  } finally {
    loading.value = false
  }
}
...
const handleDetail = (row: any) => {
  router.push({ name: 'CancelRegistryDetail', params: { id: row.id } })
}
