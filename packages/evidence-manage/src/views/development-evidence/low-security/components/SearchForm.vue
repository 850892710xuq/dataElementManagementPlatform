<template>
  <div class="dev-evidence-search-form">
    <el-form :model="formData" ref="formRef" :inline="true" class="search-form">
      ...
      <div class="form-item-wrapper">
        <label class="custom-label">安全技术：</label>
        <el-select
          v-model="formData.securityTech"
          placeholder="请选择安全技术"
          clearable
        >
          <el-option label="数据加密" value="数据加密" />
          <el-option label="权限控制" value="权限控制" />
          <el-option label="访问审计" value="访问审计" />
        </el-select>
      </div>
...
import { exportLowSecurityDevelopmentEvidenceList } from '@/api/development-evidence'
...
const handleExport = async () => {
  try {
    const blob = await exportLowSecurityDevelopmentEvidenceList({ ...formData })
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `低安全级别开发存证列表_${new Date().getTime()}.xlsx`
