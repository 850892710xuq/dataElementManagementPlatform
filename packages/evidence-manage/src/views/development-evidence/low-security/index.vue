<template>
  <div class="low-security-evidence-container">
    <!-- 查询区域 -->
...
import { getEvidenceList, getLowSecurityEvidenceDetail } from '@/api/evidence';
import type { EvidenceQueryParams, LowSecurityEvidence } from '@/types/evidence';
...
const evidenceList = ref<LowSecurityEvidence[]>([]);
const total = ref(0);

// 详情弹窗
const dialogVisible = ref(false);
const currentEvidence = ref<LowSecurityEvidence | null>(null);
...
// 查看详情
const handleViewDetail = async (row: LowSecurityEvidence) => {
  try {
...
.low-security-evidence-container {
  .search-card {
    margin-bottom: 20px;
