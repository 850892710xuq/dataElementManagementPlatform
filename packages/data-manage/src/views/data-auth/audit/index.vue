<template>
  <div class="data-auth-audit-container">
    <!-- 搜索过滤栏 -->
      ...
      >
        ...
        <el-table-column prop="status" label="申请状态" width="120">
          <template #default="scope">
            <el-tag type="warning" v-if="scope.row.status === 'initial_review' || scope.row.status === 'center_review'">
              待审核
            </el-tag>
            <el-tag :type="getStatusType(scope.row.status)" v-else>
          ...
          <template #default="scope">
            <div class="table-actions">
              ...
              <el-button
                ...
                class="action-btn"
              >
                <el-icon><CircleCheck /></el-icon>初审
              </el-button>
              <el-button
...
import { Search, Refresh, View, CircleCheck, Finished } from '@element-plus/icons-vue'
import AuthDetail from '../components/AuthDetail.vue'
import AuthReview from '../components/AuthReview.vue'

// 用户角色（实际项目中应该从用户信息或权限系统获取）
const userRole = ref('admin') // operator: 运营方，provider: 提供方，admin: 管理中心
  ...
  try {
    ...
    setTimeout(() => {
      ...
      const mockData = Array.from({ length: 10 }, (_, index) => {
        // 只生成待审核的数据
        const statusList = ['initial_review', 'center_review']
    ...
    console.error('获取数据授权审核列表失败', error)
    tableLoading.value = false
  }
}
